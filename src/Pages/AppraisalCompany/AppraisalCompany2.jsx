...
import home1 from "../../Images/AppraisalCompany.png";
...
const validationSchema_6_media = yup.object().shape({
  client_contact: yup.array()
    .min(1, "Please select at least one Type of Contact to you")
    .required("clients to contact you is required"),
...
export default function AppraisalCompany2() {
  ...
  const {
    ...
    resolver: yupResolver(step1Schema), // Initial schema for step 1
  });
  ...
  const nextStep = async (e) => {
    if (step === 1) {
      ...
      try {
        // Assuming you have these state variables or values
        await validationSchema_3.validate({ selectedAve_time, selectedService_provide, selectedPrice_start_from }, { abortEarly: false });
        setStep(step + 1);
        setErrors({});
      } catch (err) {
        const newErrors = {};
      ...
      try {
        ...
        if (socialmedia) {
          await validationSchema_6_media.validate(formData_2, { abortEarly: false });
          setStep(step + 1);
          setErrors({});
        } else {
  ...
  const handleSave = async () => {
    ...
    formData.append(
      "appraisal_per_month", appraisals || ""
    );
                  ...
                  <>
                    ...
                    <form className="space-y-6 sm:space-y-6 text-[14px] min-[1441px]:text-[18px] font-normal">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <FileUploadEco placeholder='Upload Profile Photo' onFileSelect={(file) => setProfileImg(file)} selectedFile={profileImg} />
                     ...
                     cursor-pointer text-[14px] sm:text-[18px] font-normal"
                          onClick={() => nextStep({ firstName, lastName, companyName, phonenumeber, email, address1 })}
                          type="button"
                        >
                          {step === 5 ? "Submit" : "Next"}
                        </button>
                  ...
                  <>
                    ...
                    <div className="space-y-6 sm:space-y-6 text-[14px] min-[1441px]:text-[18px] font-normal">
                      ...
                      <div className="flex flex-col gap-2">
                        {" "}
                        <label className="text-gray-800 block mb-1">
                          How many appraisals do you typically complete per
