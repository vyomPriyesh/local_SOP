...
import home1 from "../../Images/RealEstateAgent.png";
import {
  CheckBoxButton,
  CheckBoxGroup,
  CustomInput,
...
const step1Schema = (checkemail) => yup.object().shape({
  ...
  profileImg: yup.string().required("profile is required"),
...
const validationSchema = yup.object().shape({
  ...
  selectedYearexperience: yup.string().required("Years of Experience is required"),
...
const validationSchema_insurance = yup.object().shape({
  ...
  selectedCountry: yup
    ...
    .required("Country selection is required"),
  selectedYearexperience: yup.string().required("Years of Experience is required"),
  licenseno: yup.string().required("licenseno is required"),
  selectedlicenseState: yup
    .array()
    .min(1, "Please select at least one country")
    .required("State Licensed  is required"),
  selectedIndustryMembership: yup
    .array()
    .min(1, "Please select at least one country")
    .required("Industry Memberships is required"),
...
const validationSchema3 = yup.object().shape({
  //specific_neighborhoods: yup.string().required("specific neighborhoods is required"),
  selectedProperty_types: yup
    .array()
    .min(1, "Please select at least one country")
    .required("property type  is required"),
  selectedPrice_range: yup.string().required("price range  is required"),
})
...
const validationSchema4 = yup.object().shape({
  selectedService_provide: yup
    .array()
    .min(1, "Please select at least one country")
    .required("ervices do you provide  is required"),
  //selectedAve_time: yup.string().required("average turnaround time is required"),
})


const validationSchema5 = yup.object().shape({
  selectedre_time: yup.string().required("average response time is required"),
  selectedcloseperyear: yup.string().required("close per year time is required"),
})

const validationSchema5_bilingualData = yup.object().shape({
  selectedre_time: yup.string().required("average response time is required"),
  selectedcloseperyear: yup.string().required("close per year time is required"),
  bilingualData: yup
    .array()
...
const validationSchema7_media = yup.object().shape({
  client_contact: yup
    .array()
    .min(1, "Please select at least one clients to contact")
    .required("clients to contact is required"),
...
export default function RealEstateAgent2() {
  ...
  const [isLicensed, setIsLicensed] = useState('no');
  ...
  const [homeBuyers, setHomebuyers] = useState(0)
  ...
  console.log(selectedlicenseState)
  ...
  const nextStep = async (e) => {
    ...
    if (step === 1) {
      ...
      try {
        ...
        if (isLicensed === 'yes') {
          await validationSchema_insurance.validate(formData2, { abortEarly: false });
          setStep(step + 1);
          setErrors({});
        } else {
      ...
      try {
        ...
        if (isbilingual_services == 'yes') {
          await validationSchema5_bilingualData.validate(formData2, { abortEarly: false });

        } else {
          await validationSchema5.validate(formData, { abortEarly: false });
        }
        setStep(step + 1);
        setErrors({});
      } catch (err) {
      ...
      try {
        ...
        if (socialmedia) {
          await validationSchema7_media.validate(formData_2, { abortEarly: false });
          setStep(step + 1);
          setErrors({});
        } else {
          await validationSchema7.validate(formData, { abortEarly: false });
      ...
      try {
        await validationSchema8.validate(formData, { abortEarly: false });
        setErrors({});
        handleSave()
      } catch (err) {
        const newErrors = {};
  ...
  const closeperyear = ["<10", "10-25", "25-50", "50-100", "100+"];
  const Industry = ["ALTA", "NAR", "Local Real Estate Boards", "Etc"];
  // Previous Step
  const prevStep = () => {
  ...
  const handleLisStateChange = (selectedName) => {
    ...
    const selectedStatesData = states.filter((country) =>
  ...
  const handleSave = async () => {
    ...
    formData.append(
      "property_types",
      Object.values(selectedProperty_types).join(",") || ""
    );
    formData.append("specific_neighborhoods",  Object.values(specific_neighborhoods).join(",")  || "");
    formData.append("price_range", selectedPrice_range || "");
    ...
    formData.append("homebuyers", homeBuyers || "");
    ...
    formData.append("transaction_per_year", selectedcloseperyear || "");
    formData.append("response_time", selectedre_time || "");
    formData.append(
      "bilingual_services",
  ...
  const addTag = (value) => {
    ...
    if (newTag && !specific_neighborhoods.includes(newTag)) {
      setSpecific_neighborhoods((prev) => [...prev, newTag]);
      setTagInput("");
    }
  };

  const removeTag = (indexToRemove) => {
    setSpecific_neighborhoods(specific_neighborhoods.filter((_, index) => index !== indexToRemove));
  };
             ...
             checked:before:top-1/2 checked:before:-translate-x-1/2 checked:before:-translate-y-1/2"
                ...
                {step === 3 && (
                  <>
                    <div className="flex justify-between items-center mb-4 text-[22px]  min-[1025px]:text-[26px] min-[1441px]:text-[32px] p-1 text-center font-semibold">
                      {step > 1 && <FaArrowLeftLong onClick={prevStep} />}
                      <h2 className=" ">Areas & Markets Served</h2>
                      <span className="text-gray-500  ">
                        <span className="text-[var(--primary-color)]">
                          {step}
                        </span>
                    ...
                    <form className="space-y-6 sm:space-y-6 text-[14px] min-[1441px]:text-[18px] font-normal">
                      <div className="flex flex-col gap-2">
                        {" "}
                        <label className="text-gray-800 block mb-1">
                          Do you specialize in any specific neighborhoods?{" "}
                          {/* <span className="text-red-500">*</span> */}
                        </label>
                        ...
                        <CustomTextArea
                          ...
                          tags={specific_neighborhoods}
                          removeTag={removeTag}
                        />
                        {/* {error.specific_neighborhoods && <p className="text-sm text-red-500">{error.specific_neighborhoods}</p>} */}
                      </div>

                      <div className="flex flex-col gap-2">
                        {" "}
                        <label className="text-gray-800 block mb-1">
                          Do you focus on specific property types?
                          <span className="text-red-500">*</span>
                        </label>
                        <Dropdown1
                          options={specificproperty}
                          onChange={(value) => setSelectedProperty_types(value)}
                          placeholder="Residential, Commercial, Luxury..."
                        />
                        {error.selectedProperty_types && <p className="text-sm text-red-500">{error.selectedProperty_types}</p>}
                      </div>
                      <div className="flex flex-col gap-2">
                        {" "}
                        <label className="text-gray-800 block mb-1">
                          What is your average price range for transactions?
                          <span className="text-red-500">*</span>
                        </label>
                        <Dropdown2
                          options={transactions}
                          onChange={(value) => setSelectedPrice_range(value)}
                          placeholder="<$100K, $100K-$250K...."
                        />
                ...
                {step === 4 && (
                  <>
                    <div className="flex justify-between items-center mb-4 text-[22px]  min-[1025px]:text-[26px] min-[1441px]:text-[32px] p-1 text-center font-semibold">
                      {step > 1 && <FaArrowLeftLong onClick={prevStep} />}
                      <h2 className=" ">Services & Expertise</h2>
                      <span className="text-gray-500  ">
                        <span className="text-[var(--primary-color)]">
                          {step}
                        </span>
                    ...
                    <div className="space-y-6 sm:space-y-6 text-[14px] min-[1441px]:text-[18px] font-normal">
                      {/* <div className="flex flex-col gap-2">
                        {" "}
                        <label className="text-gray-800 block mb-1">
                          What is your average turnaround time for closer?
                          <span className="text-red-500">*</span>
                        </label>
                        <Dropdown2
                          options={average}
                      ...
                      <div className="flex flex-col gap-2">
                        <div className="">
                          {" "}
                          <p>Do you work with first-time homebuyers?</p>
                          <div className="flex ms-2 mt-2">
                            {" "}
                            <label className="flex items-center space-x-2 cursor-pointer">
                              <input
                                ...
                                name="consultations"
                                value="yes"
                                checked={homeBuyers == 1}
                                onChange={() => setHomebuyers(1)}
                                className="appearance-none w-6 h-6 rounded-md border-2 border-white
             bg-[#E5E6EB] shadow-[1px_1px_3px_0px_rgba(0,0,0,0.2),-1px_-1px_3px_0px_rgba(255,255,255,0.8)]
             checked:bg-blue-500 checked:border-white
             checked:shadow-[4px_4px_8px_0px_rgba(0,0,0,0.3),-4px_-4px_8px_0px_rgba(255,255,255,1)] 
             ...
             checked:before:top-1/2 checked:before:-translate-x-1/2 checked:before:-translate-y-1/2"
                            ...
                            <label className="flex items-center space-x-2 cursor-pointer ms-2">
                              <input
                                ...
                                name="consultations"
                                value="no"
                                checked={homeBuyers == 0}
                                onChange={() => setHomebuyers(0)}
                                className="appearance-none w-6 h-6 rounded-md border-2 border-white
             bg-[#E5E6EB] shadow-[1px_1px_3px_0px_rgba(0,0,0,0.2),-1px_-1px_3px_0px_rgba(255,255,255,0.8)]
             checked:bg-blue-500 checked:border-white
             checked:shadow-[4px_4px_8px_0px_rgba(0,0,0,0.3),-4px_-4px_8px_0px_rgba(255,255,255,1)] 
                  ...
                  <>
                    ...
                    <div className="space-y-6 sm:space-y-6 text-[14px] min-[1441px]:text-[18px] font-normal">
                      <div className="flex flex-col gap-2">
                        ...
                        <Dropdown2
                          options={closeperyear}
                          placeholder="<10, 10-25..."
                          onChange={(value) => setSelectedcloseperyear(value)}
                        />
                        {error.selectedcloseperyear && <p className="text-sm text-red-500">{error.selectedcloseperyear}</p>}
                      </div>

                      <div className="flex flex-col gap-2">
                        {" "}
                        <label className="text-gray-800 block mb-1">
                          What is your average response time to inquiries?
                          <span className="text-red-500">*</span>
                        </label>
                        <Dropdown2
                          options={average}
                          placeholder="Within an hour, Same day..."
                          onChange={(value) => setSelectedre_time(value)}
                        />
                        {error.selectedre_time && <p className="text-sm text-red-500">{error.selectedre_time}</p>}
                      </div>
                    ...
                    <div className="space-y-6 sm:space-y-6 text-[14px] min-[1441px]:text-[18px] font-normal">
                      ...
                      {/* <div className="flex flex-col gap-2">
                        ...
                        <InputField
                          ...
                          onChange={(e) =>
                            setintroduction_video(e.target.value)
                          }
                        />
                      </div> */}
